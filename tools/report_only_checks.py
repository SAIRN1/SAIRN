...
NOT_PROMOTED = [
    ...
    ('assurance_case.py', 'ITEM 5, AND PROMOTING IT WOULD INVERT WHAT IT IS '
     'FOR. It exits 1 today because five of eight developed goals are NOT '
     'SUPPORTED, and that is the honest state of the platform rather than a '
     'defect in the tool -- a runner entry would report the same five on every '
     'push until somebody silenced it, and silencing an assurance case is worse '
     'than not having one. IT ALSO MUST NOT BECOME A GATE: a case that can '
     'block a push acquires a constituency for being green, and the value here '
     'is entirely in its willingness to say NOT SUPPORTED and UNDEVELOPED. '
     'Promote it the day a majority of its goals are supported and the '
     'remaining ones are the news. Held by tests/run_assurance_case_probe.py, '
     'whose sharpest arm injects a failing evidence command and requires the '
     'top verdict to follow.'),
    ('risk_event_tree.py', 'ITEM 84, and its output does not change until the '
     'INPUTS do. The four barrier bands are judgements anchored to measurements '
     'that move on the scale of weeks, so a runner entry would print an '
     'identical tree every push. Promote it the day there is incident data to '
     'fit a rate to -- which is also the day its numbers stop being judgements. '
     'IT REPORTS ITS RANKING AS NOT STABLE TODAY and refuses to pick the work '
     'on that basis, which is the behaviour to preserve if it is ever wired '
     'anywhere. Held by tests/run_risk_event_tree_probe.py, which re-derives '
     'the worst state by algebra and the range by dense sampling rather than '
     'trusting the corner search.'),
    ('dora_metrics.py', 'A MEASUREMENT OF THE BUILD METHOD, NOT A CHECK ON IT. '
     'There is no pass or fail here: deployment frequency is a statement about '
     'batch size, and wiring it into a runner would create pressure on a number '
     'that is not a goal. It also exits 1 by design while lead time is '
     'NOT MEANINGFUL on a trunk topology, which is a permanent property of this '
     'repository rather than something a push can fix. Read it deliberately, on '
     'a cadence, and read the COVERAGE beside the change failure rate rather '
     'than the rate alone. Held by tests/run_dora_metrics_probe.py.'),
    ('retry_policy_audit.py', 'ITEM 81, AND ITS ANSWER IS A ZERO THAT WILL '
     'NOT MOVE. There are no true retries on a remote call in live code, so the '
     'only two constructs it reports are a poll and an agent turn loop, and a '
     'runner entry would print the same two lines on every push. Promote it the '
     'day somebody ADDS a retry, which is exactly when the backoff question '
     'starts having an answer that can change. TWO EARLIER ATTEMPTS AT THIS '
     'ITEM WERE RETRACTED and both retractions are the reason it is careful: a '
     'regex pass reported 24 of 25 api/ files on the strength of the word '
     '"retry" in comments, and the first run of this tool itself '
     'reported 24 retry '
     'constructs that were iteration, pagination, queue drains and polls. Its '
     'blind lock now asserts the ROLE as well as the policy, which is what '
     'caught the second class. Held by tests/run_retry_policy_probe.py.'),
    ('suite_control_coverage.py', 'IT REPORTS THE SAME SUITES EVERY RUN and '
     'will until somebody writes negative controls, which is months of work -- a '
     'notice whose content cannot change is a notice that stops being read, the '
     'same argument recorded for reliability_growth.py above. It is also NOT a '
     'list of bad suites: a pure-function unit test over a table of inputs has '
     'little to sabotage, so promoting it would put a judgement call into a '
     'pass/fail column. Promote it the day the number is small enough that a '
     'change in it means something. Held by '
     'tests/run_suite_control_coverage_probe.py, whose first arm bounds it from '
     'above with a deliberately dumber search so it cannot over-claim coverage.'),
    ('first_article_check.py', 'ITEM 47, AND THE RESTRAINT IS THE POINT. It '
     'passes today with three inspections recorded and nothing since the '
     'requirement date missing one, so a runner entry would be silent -- but the '
     'reason it is not promoted is not that it is quiet. THE CLAIM-TO-ARM '
     'MAPPING IS HUMAN AND MUST STAY HUMAN: word-overlap scoring of prose claims '
     'against prose arm labels returned 38% with five false positives out of '
     'five on 2026-09-14, and the session that measured that refused to promote '
     'its extractor for exactly this reason. A gate that can only check the '
     'BOOKKEEPING around a judgement should not be able to refuse a push on the '
     'judgement itself. Promote it the day the push gate should start requiring '
     'an inspection record for new tools, which is a policy decision about how '
     "much a new tool costs to ship and is Michael's to make, not a tool's. "
     'Held by '
     'tests/run_first_article_probe.py.'),
