...
PURPOSES = {
    ...
    'coding_rule_discovery.py': ('CHECKER',
        ...
        'convention'),
    'session_recheck_coverage.py': ('CHECKER',
        'a session gate that never asks again. verifySessionToken() proves a '
        'token was minted by us and has not expired; it proves NOTHING ABOUT '
        'NOW, because there is no server-side session store on this platform, '
        'so a credential DEACTIVATED after the token was issued keeps working '
        'until exp -- up to 12h. Deactivation is the one control an owner has '
        'over somebody who has just left. COUNTS BOTH MECHANISMS, and that is '
        'the whole reason it is trustworthy: credentialStillActive() is the '
        'explicit re-check, and an employee-row load filtered active=eq.true '
        'is what every api/*-auth.js does inside loadEmployee(). Counting only '
        'the first reports 3 of 222 gates and would accuse seventeen correct '
        'files -- that was this tool own first number and an arm of its probe '
        'pins against it. IT IS A RATCHET, NOT A PASS/FAIL, pinned to '
        'docs/session-recheck-coverage.json: the honest state is not closed, '
        'and a check that simply failed would sit permanently red, which is the '
        'state that gets scrolled past. OK means "no worse than pinned" and it '
        'says so on every run; an absent or unparseable pin file is exit 2 '
        'COULD NOT TELL, never 0. It reports PARTIAL separately from covered, '
        'which is the distinction a presence check cannot make -- '
        'api/sd-data.js has 132 gates and ONE re-check, so "this file has the '
        'mechanism" is true and 131 gates do not use it. WHAT IT CANNOT DO is '
        'decide whether a given gate NEEDS the re-check: a read-only or '
        'self-scoped route is a weaker case than a money write, and that '
        'judgement is not automatable and is not attempted. Control: '
        'tests/run_session_recheck_coverage_probe.py, nine sections driven in '
        'BOTH directions -- a new unprotected gate regresses and a new '
        'PROTECTED one is silent, because a ratchet that fired on every new '
        'gate would be switched off within a day.'),
    'blob_conversion_coverage.py': ('CHECKER',
        'how many stored jsonb blobs are built through api/_lib/blob.js and how '
        'many still hand the raw request payload to the store. IT EXISTS '
        'BECAUSE A COMPLETION PROOF MEASURED SOMETHING ELSE: item 94\'s blob '
        'migration was declared COMPLETE with `grep -c "Object.assign({}, '
        'payload)" 12 -> 0`, and `.` is a WILDCARD, so that pattern needs some '
        'character then `)` right after `payload` -- while every site left '
        'behind is `Object.assign({}, payload, {...})`, a COMMA. The pattern '
        'could not match the remainder, so it was guaranteed to reach 0 the '
        'moment the sites it COULD match were converted. Re-measured by shape '
        'rather than punctuation: 77 unconverted against 24 converted, so the '
        'migration was reported complete at under a quarter done. A site is a '
        '`data:` property beside a license_hash -- that pairing is what makes '
        'it a tenant-scoped row rather than a local object called data -- and '
        'sites are classified CONVERTED / RAW / SPREAD / NESTED / BUILT. BUILT '
        'IS NOT A GAP (blob.js says a field-by-field blob is already immune) '
        'and NESTED is a different argument, so neither is folded into the '
        'unconverted figure. WHAT IT CANNOT DO IS CHOOSE THE COLUMN KEYS: which '
        'payload fields are real columns is a fact about each table, which is '
        'why blob.js keeps that list per-branch -- sd_slabs and every mech_ '
        'record keep `id` INSIDE data on purpose because their reads return '
        '`x.data` whole -- so it reports which sites are unconverted and never '
        'what to pass. A RATCHET pinned to docs/blob-conversion-coverage.json, '
        'because the honest state is nowhere near closed and a check that '
        'simply failed would sit permanently red; an absent, unparseable or '
        '`unconverted`-less pin is exit 2 COULD NOT TELL, and so is a scan that '
        'finds ZERO sites, since the write-body shape moving must not read as '
        '"nothing to convert". ITS OWN FIRST THREE VERSIONS COMMITTED THE DEFECT '
        'IT WAS BUILT TO CATCH, each recorded at the line that caused it: a '
        '`[^,\\n]+` capture truncated the inline spread at the first comma and '
        'reported SPREAD 0; a FILE-WIDE set of storedBlob-bound names scored the '
        'unconverted rf_claims write as converted because `dataBlob` is bound '
        'twice; and the nearest-binding fix then INHERITED a stale verdict for a '
        'name rebound to `Object.assign({}, curData, ...)`, overcounting its own '
        'remainder. Control: tests/run_blob_conversion_coverage_probe.py, eight '
        'sections both directions -- each shape recognised as itself, the '
        'strip-then-override fix pattern NOT read as a gap, all three of those '
        'defects reproduced as fixtures, and a final section on the real api/ '
        'asserting it wrote nothing by bytes AND mtime.'),
    'tiering_recheck.py': ('CHECKER',
        'which tier assignments the platform\'s own review history has earned '
        'the right to DOUBT. Methodology item 26 phase 2, and it was gated on '
        'Batch 3 for a reason that was not technical: a re-check driven by '
        'review history correlates nothing until there IS review history, and a '
        'tool built early would have reported confidence it had not earned. It '
        'asks EVIDENCE OF SURPRISE, never "is this tier right" -- a tier is a '
        'judgement about consequence and this tool has no access to '
        'consequence. IT PROPOSES AND MAY NEVER APPLY (cross-domain '
        'disciplines item 11) and must not be given the ability: a tier '
        'promotion is not even a cell edit, because SC_TIER_A_SOFT_DELETE_ONLY '
        'and the write gates DERIVE from the Tier A list, so an A withdraws a '
        'resource\'s `delete` verb and moves its client remove path. ITS FIRST '
        'CRITERION WAS TOO WIDE AND ITS OWN OUTPUT SAID SO: app signal alone '
        'returned 95 candidates, every B row in any app with a severe find, '
        'all of them reviews=0 -- furniture. The claim needs the reviews to '
        'have LOOKED AT THE ROW, so unreviewed rows get their own '
        'review-SCHEDULING bucket rather than being folded in to look '
        'thorough. THE HEADLINE IS THE BLIND SPOT, printed before the empty '
        'list it explains: the review gate opens an obligation when TIER A '
        'code changes, so a row left at B is never reviewed and no review can '
        'surprise anybody about it -- UNDER-tiering is the direction this '
        'mechanism cannot see and the direction that leaves a real record '
        'under-protected. THE LINK IS PER APP, NOT PER RESOURCE, because '
        'defect records carry `app` and `files` and not resource names, so one '
        'defect raises the signal for every resource in the app; the two '
        'signals are printed apart and NEVER summed, since a single score here '
        'would be the fabricated-metric shape this platform polices hardest. '
        'It reads OWNER_BY_RESOURCE by RUNNING api/_resources rather than '
        'regexing it, and a row anchor that no longer matches is exit 2 COULD '
        'NOT TELL -- not a clean run over zero rows. AND IT OVERSTATED ITS OWN '
        'CORPUS 4.7x BEFORE ANYONE ELSE READ IT: the header printed the SUM of '
        'the per-resource review counts and called it "review record(s)", so '
        '167 records naming 789 record-resource pairs announced 789 reviews -- '
        'wrong in the one direction that makes the tool look better founded '
        'than it is. Both numbers are carried and both are labelled; neither '
        'wears the other\'s name. Control: '
        'tests/run_tiering_recheck_probe.py, eight sections both directions -- '
        'a reviewed B row in a surprising app IS a candidate and the SAME row '
        'unreviewed is not, severity and detection_method must both match, '
        'EARNING and UNEXAMINED stay distinct, one record naming three '
        'resources reports 1 and not 3 (with the real-register counts computed '
        'in the probe rather than hardcoded, plus an arm asserting the two '
        'numbers still DIFFER so the check cannot quietly stop testing), every '
        'missing source is exit 2, and a final arm asserts it wrote NOTHING to '
        'any real register by bytes AND mtime so a write-then-restore fails '
        'too.'),
    ...
    'confidentiality_candidate_flagger.py': ('ADVISORY',
        'the B/C rows of docs/CRITICALITY-TIERS.md that a person must read '
        'before the two-axis migration scores them for confidentiality -- 288 '
        'rows narrowed to a list short enough to read by hand. IT FLAGS AND '
        'DOES NOT SCORE, which is not a limitation to be closed later: the '
        'register\'s own header says a tier asserted with no evidence is a '
        'label, so a script cannot assign one. It does not edit the register, '
        'writes no tier anywhere, and exits 0 at any count. THE THING IT '
        'ACTUALLY CATCHES IS A SIGNAL THAT MEASURED PROXIMITY INSTEAD OF '
        'OWNERSHIP, because both of its own signals did. Asking only "is there '
        'a server-side check" flagged 173 of 288 rows, since whole apps have no '
        'session gate by a RECORDED decision (SD_LOCAL_RESOURCES says so in its '
        'own header) -- the signal is the DISAGREEMENT between client and '
        'server, not the absence. And the payload signal scanned 400 characters '
        'after the resource name for a PII/PHI word, which in a single-file app '
        'runs straight into the declaration next door: all 17 rows it flagged '
        'were wrong, ten sen_* rows for `pay_rate` off a cache array containing '
        'the DIFFERENT resource \'sen_pay_rates\', four leg_* rows for '
        '`decedent` off one shared helper reading a CASE. It now reads the '
        'resource\'s own record literal, balance-parsed and string-blanked. '
        'WHERE THE APP HANDS IT NO READABLE LITERAL THE PAYLOAD SIGNAL CANNOT '
        'BE ASKED AT ALL, returned as a separate boolean rather than an empty '
        'field list so a could-not-tell is never read as a clean payload, and '
        'printed on its own lines in the report. --self-test carries six locked '
        'fixtures in BOTH directions, because a count that fell is '
        'indistinguishable from a signal that stopped working.'),
    ...
    'suite_control_coverage.py': ('CHECKER', 'which JavaScript test suites have a NEGATIVE CONTROL -- a probe that breaks the source and asserts the suite goes red -- and which have only ever been green. Applied, unique, EFFECTIVE: sabotage_control_check.py asks the first, mutation_anchor_check.py the second, and both name this third as a gap they cannot close. THREE CONTROL SHAPES, because a census whose universe is one spelling reports every other spelling as absent -- and this counter has now been wrong that way twice: a separate probe FILE, an INLINE mutation inside the suite (2026-09-17), and a MODULE STUB that replaces a require.cache entry rather than sabotaging source text (2026-09-18), which is how api/sd-data-dental-ledger-validation.test.js carried 47 mutation arms while reading as never sabotaged. The stub shape is deliberately narrow: an ASSIGNMENT into require.cache, not a delete, and only when the mutation is announced in code rather than in a comment -- 22 suites install into require.cache and 21 are stubbing a dependency to make a handler testable, 10 more only delete, and reading the announcement from raw source would admit 5 files whose only mention is prose about some other control. Each shape carries its own sentinel in the output and its own applied-check: a textual diff for a replace, and for a stub a BEHAVIOURAL assertion that the neutered run differs, because a module swap has no before-string to compare and Node reports a failed bind as a warning'),
    ...
    'load_compliance_seed.py': ('LIVE', 'a SAIRNcare compliance seed that is committed and INERT -- it loads the rules to a licence and proves it by driving the engine on IDENTICAL inputs before and after, failing when the answer did not move, because a loader exit code is not evidence'),
    'plugin_upgrade_check.py': ('LIVE', 'a plugin reported as BEHIND when it was just upgraded, and one reported CURRENT that never was -- it compares VERSIONS read off the installed plugin.json and the marketplace pin, and never reads gitCommitSha, which `claude plugin update` leaves pointing at the old commit'),
    'row_count_baseline.js': ('LIVE', 'a restore that lost every row in a table and compared EQUAL -- it writes the per-table baseline restore_coherence_check.js has taken --baseline for since 2026-09-14 and nothing produced, and a table it could not count is OMITTED rather than written as 0, because a 0 is what makes a wiped table pass'),
    ...
    "primitive_obsession_check.py": ("CHECKER", "a NEW occurrence of three shapes where a raw primitive crosses a boundary unparsed, each already paid for here: a measured value defended into a default (Number(x)||0 -- empty, unreadable and a legitimate zero collapse into one number, the sairndental silent-$0 shape), a config read where Number('') is 0 (a cleared env var becomes a switched-off feature that looks configured), and a locale date string stored or compared as data (toLocaleDateString does not sort and differs per viewer). 233 existing keys grandfathered; refuses (exit 2) when its own fixture lock fails OR when a whole baselined shape finds zero matches, because a detector that went blind must never look like progress. Disjoint from truthy_sum_check by construction: that one requires coercion ABSENT, these require it present or absent-but-locale"),
    ...
    'wait_for.py': ('CHECKER', "a watcher that outlives its subject -- it catches the case where the thing being "
        "waited on DIES and the wait continues for ever. Measured cost: on 2026-09-25 an inline `until grep` loop "
        "polled for FOURTEEN HOURS after the process producing its sentinel was killed, because the loop's only "
        "terminal state was the happy path. Exits 3 WATCHED PID GONE -- explicitly NOT 0 -- when the pid exits "
        "without the condition being met, 4 on timeout, 2 on bad arguments, and re-checks the condition once after "
        "seeing the pid gone so a subject's final write is not discarded as a failure. An unreadable process table "
        "is UNKNOWN rather than dead, counted and reported"),
