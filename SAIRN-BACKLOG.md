...
## SAIRNlaw AI Chain of Custody: two honest gaps between the shipped feature and its "immutable, matter-linked, always-logged" claim
...
server-side state-machine verification gate are all genuinely real —
independently re-verified, not just claimed. Two things are not yet as
solid as the feature's framing implies:
1. **Capture is client-reported, not proxy-observed.** The AI call
   (browser -> `PROXY`) and the log write (browser -> `api/law-auth.js`
   `ai_log`) are two separate, unlinked browser-originated requests, and
   the log write is fire-and-forget (not awaited by the AI response
   itself). Closing the tab immediately after the AI answer renders,
   before the log request completes, or blocking/editing the log request
   specifically, produces a real AI interaction with no record — and
   symmetrically, nothing today stops a fabricated prompt/response pair
   from being logged that was never actually generated by the API. "Every
   AI interaction is logged" is enforced client-side only.
2. **`matter_id` is an unvalidated localStorage id.** `law_matters` (like
   all of SAIRNlaw's other 19 real resources) is client-only — the server
   accepts whatever `matter_id` string the client sends with no
   ...
   license. "Matter-linked" today means "linked to whatever the user's
   own browser believed the matter was," not independently verified.

**Why deferred:** Closing gap 1 properly means moving AI capture to a
real server-side proxy path (log the interaction from the same request
that calls Claude, not a second client-fired request after the fact) —
a real architecture change, not a quick patch, and explicitly Phase 2
scope per this feature's own design spec. Closing gap 2 requires
`law_matters` to become server-backed first (the same much larger,
already-logged, not-yet-started SAIRNlaw server-sync gap this file
already tracks elsewhere for `law_trusttx` and friends) — a prerequisite
this feature can't unilaterally deliver.

**Done looks like:** Gap 1: AI interactions are logged from a real
server-side path (the proxy call itself, or a server-side wrapper around
it) rather than a second client-fired request, so the record exists
independent of what the browser does afterward. Gap 2: `matter_id` is
validated server-side against a real `law_matters` record once that
resource is server-backed. Until both land, any external description of
this feature should say "logged and matter-tagged by the user's own
client" rather than "independently verified" or "server-observed."
...
3. **No volume cap on `ai_log`.** An authenticated user can append
   unbounded rows to a table that, by design, has no delete path. Low risk
   today (single-firm, authenticated-only), flagged only because it's
   structurally uncleanable if it ever became a real problem. **Done looks
   like:** a per-license row-count or time-window cap, decided alongside
   whatever storage/retention policy the firm actually wants for a legal
   record (not an arbitrary technical limit imposed without that
   conversation).
