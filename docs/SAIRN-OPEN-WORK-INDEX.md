...
| **Platform** | `service_role` holds TRUNCATE/REFERENCES/TRIGGER (Postgres default ACL, confirmed live via `pg_default_acl`) on tables **beyond** the append-only-by-design set already fixed in `sql/append_only_grant_audit.sql` — StoneDesk's slab tables, SAIRNgrounds/MSB's tables, and most other full-CRUD app-data tables | Open — **deliberately deferred, not urgent** | unassigned | — | Same root cause and same fix shape as the append-only fix (`revoke all from service_role` then re-grant exactly what's needed), but a different risk class: TRUNCATE on a full-CRUD table doesn't defeat a specific append-only guarantee, so it doesn't ride along on that pass. `sql/append_only_grant_audit.sql`'s own Section 2 (list-free discovery sweep) already surfaces every affected table with no new query needed — this item is running that sweep's output through the same fix, scoped on its own | L |
