...
**CORRECTION, 2026-09-22 — the sentence above says finding #393 is "already routed for a build agent to fix in the code", and that is not what happened.** `api/sd-data.js` was re-read directly at the `SV_RESOURCES` block. There is still **no session gate**, and it is not an oversight waiting on a fix: the handler now carries an explicit comment recording the absence as a **decision** — *"SAIRNvet has no per-employee authentication at all -- `role` is a self-selected dropdown, never server-verified. A session gate here would gate on a session that does not exist."* The fix that landed was the **documentation** of the absence, not a gate. That strengthens this section rather than weakening it: the boilerplate was not describing something that had since regressed, it was asserting a code fact **that had never been true and that nothing ever checked**. The row count reconciles exactly — 41 `SV_RESOURCES` keys, 41 `sv_` rows in the register, 44 names in `api/_resources/sairnvet.js` less the three documented `notSynced` exclusions (`sv_audit_backed`, `sv_examrooms_turnover`, `sv_settings`); there is no discrepancy. The real gate remains open work on SAIRNvet's missing auth subsystem, for all forty-one at once.
...
**ONE OPEN QUESTION THIS SECTION'S OWN REPLACEMENT SENTENCE RAISES, found by making it enforceable.** The proposed Confidentiality-B default below opens *"Internal, role-restricted data…"* — which is itself an access-control claim, weaker than *"employee-auth-gated"* but the same kind of assertion, and equally unverified by this table. The check as landed deliberately catches only the `auth-gated` / `session-gated` shape that finding #393 actually falsified, so the sentence below passes it as written. Whether *"role-restricted"* should stay is a judgement for the spec's owner, not something a build agent should quietly widen a regex to decide.
...
- **Confidentiality-B default:** *"Internal, role-restricted data with no elevated confidentiality class — no PII, PHI, privileged communication, or financial-account detail on this row. Classified by the stated B rule rather than individually read."*
