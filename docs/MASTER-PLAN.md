...
| `sairnbuild` | 32 | ✅ | 5 | 4 | 1 | — |
| `sairncare` | 13 | ✅ | 21 | 4 | 1 | — |
| `sairncash` | 0 | ✅ | 3 | 1 | 0 | **no fault probe** |
...
**Platform totals: 386 resources owned by an app, 155 test files attributed to one, 69 of those traced, 17 fault probes.**

**And the denominators that are NOT the same thing**, stated because conflating them is what went wrong: there are **425** test files on disk in total and **219** of them are traced — most are not attributed to any single app, so the per-app `traced` column above sums to less. A rate over the subset you looked at is not a rate.
...
**WORST CASE: 504.** RSS for context: 341.
