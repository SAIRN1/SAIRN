...
| `sairnbuild` | 32 | ✅ | 5 | 4 | 1 | — |
| `sairncare` | 13 | ✅ | 21 | 4 | 1 | — |
| `sairncash` | 0 | ✅ | 4 | 1 | 0 | **no fault probe** |
...
**Platform totals: 386 resources owned by an app, 156 test files attributed to one, 69 of those traced, 17 fault probes.**

**And the denominators that are NOT the same thing**, stated because conflating them is what went wrong: there are **426** test files on disk in total and **219** of them are traced — most are not attributed to any single app, so the per-app `traced` column above sums to less. A rate over the subset you looked at is not a rate.
...
|---|---|---|---|
| `suites` | 270 | UNDER-counts | test files on disk attributed to no single app by path |
...
**WORST CASE: 505.** RSS for context: 341.
...
```
  ...
  test files on disk               426   tests/**, api/*.test.js
  tests traced to a requirement    225   traceability_matrix.traced()
