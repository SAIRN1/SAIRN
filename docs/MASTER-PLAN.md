...
|---|---|---|---|---|---|---|
| `sairnbiz` | 10 | ✅ | 5 | 5 | 0 | **no fault probe** |
...
**Platform totals: 382 resources owned by an app, 77 test files attributed to one, 31 of those traced, 8 fault probes.**

**And the denominators that are NOT the same thing**, stated because conflating them is what went wrong: there are **355** test files on disk in total and **160** of them are traced — most are not attributed to any single app, so the per-app `traced` column above sums to less. A rate over the subset you looked at is not a rate.
...
|---|---|---|---|
| `suites` | 278 | UNDER-counts | test files on disk attributed to no single app by path |
| `traced` | 195 | UNDER-counts | test files no source ties to a stated requirement |
| `fault` | 22 | UNDER-counts | test files that write to a tracked app file and declare neither a MUTATIONS block nor a *_fault_probe.py name |
| `tiered` | 0 | no contribution | binary and complete -- criticality_tier_check either raised something for an app or did not |

**WORST CASE: 495.** RSS for context: 340.
...
```
  ...
  tests traced to a requirement    165   traceability_matrix.traced()
