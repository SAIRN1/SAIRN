...
**Platform totals: 387 resources owned by an app, 170 test files attributed to one, 115 of those traced, 29 fault probes.**

**And the denominators that are NOT the same thing**, stated because conflating them is what went wrong: there are **491** test files on disk in total and **388** of them are traced — most are not attributed to any single app, so the per-app `traced` column above sums to less. A rate over the subset you looked at is not a rate.
...
|---|---|---|---|
| `suites` | 321 | UNDER-counts | test files on disk attributed to no single app by path |
...
**WORST CASE: 453.** RSS for context: 338.
...
```
  ...
  tests traced to a requirement    394   traceability_matrix.traced()
  declared fault probes             34   MUTATIONS blocks + *_fault_probe.py + *_mutation_control.js + tests/faults/*.js
