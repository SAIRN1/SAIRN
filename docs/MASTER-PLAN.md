...
**Platform totals: 387 resources owned by an app, 171 test files attributed to one, 171 of those traced, 30 fault probes.**

**And the denominators that are NOT the same thing**, stated because conflating them is what went wrong: there are **493** test files on disk in total and **493** of them are traced — most are not attributed to any single app, so the per-app `traced` column above sums to less. A rate over the subset you looked at is not a rate.
...
```
  ...
  tests traced to a requirement    499   traceability_matrix.traced()
