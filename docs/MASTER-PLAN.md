...
**Platform totals: 387 resources owned by an app, 170 test files attributed to one, 122 of those traced, 29 fault probes.**

**And the denominators that are NOT the same thing**, stated because conflating them is what went wrong: there are **492** test files on disk in total and **422** of them are traced — most are not attributed to any single app, so the per-app `traced` column above sums to less. A rate over the subset you looked at is not a rate.
...
**WORST CASE: 421.** RSS for context: 331.
...
```
  ...
  test files on disk               492   tests/**, api/*.test.js
  tests traced to a requirement    428   traceability_matrix.traced()
