...
**Platform totals: 387 resources owned by an app, 181 test files attributed to one, 177 of those traced, 36 fault probes.**

**And the denominators that are NOT the same thing**, stated because conflating them is what went wrong: there are **515** test files on disk in total and **501** of them are traced — most are not attributed to any single app, so the per-app `traced` column above sums to less. A rate over the subset you looked at is not a rate.
...
|---|---|---|---|
| `suites` | 334 | UNDER-counts | test files on disk attributed to no single app by path |
| `traced` | 14 | UNDER-counts | test files no source ties to a stated requirement |
...
**WORST CASE: 377.** RSS for context: 336.
...
```
  ...
  test files on disk               515   tests/**, api/*.test.js
  tests traced to a requirement    507   traceability_matrix.traced()
  declared fault probes             48   MUTATIONS blocks + *_fault_probe.py + *_mutation_control.js + tests/faults/*.js
