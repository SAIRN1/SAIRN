...
**Platform totals: 387 resources owned by an app, 176 test files attributed to one, 174 of those traced, 33 fault probes.**

**And the denominators that are NOT the same thing**, stated because conflating them is what went wrong: there are **503** test files on disk in total and **498** of them are traced — most are not attributed to any single app, so the per-app `traced` column above sums to less. A rate over the subset you looked at is not a rate.
...
|---|---|---|---|
| `suites` | 327 | UNDER-counts | test files on disk attributed to no single app by path |
| `traced` | 5 | UNDER-counts | test files no source ties to a stated requirement |
...
**WORST CASE: 361.** RSS for context: 328.
...
```
  ...
  test files on disk               503   tests/**, api/*.test.js
  tests traced to a requirement    504   traceability_matrix.traced()
  declared fault probes             39   MUTATIONS blocks + *_fault_probe.py + *_mutation_control.js + tests/faults/*.js
