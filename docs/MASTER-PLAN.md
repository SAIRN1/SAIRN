...
**Platform totals: 387 resources owned by an app, 172 test files attributed to one, 172 of those traced, 31 fault probes.**

**And the denominators that are NOT the same thing**, stated because conflating them is what went wrong: there are **494** test files on disk in total and **494** of them are traced — most are not attributed to any single app, so the per-app `traced` column above sums to less. A rate over the subset you looked at is not a rate.
...
| `traced` | 0 | UNDER-counts | test files no source ties to a stated requirement |
...
**WORST CASE: 351.** RSS for context: 323.
...
```
  ...
  test files on disk               494   tests/**, api/*.test.js
  tests traced to a requirement    500   traceability_matrix.traced()
  declared fault probes             36   MUTATIONS blocks + *_fault_probe.py + *_mutation_control.js + tests/faults/*.js
