...
**Platform totals: 387 resources owned by an app, 182 test files attributed to one, 178 of those traced, 39 fault probes.**

**And the denominators that are NOT the same thing**, stated because conflating them is what went wrong: there are **517** test files on disk in total and **502** of them are traced — most are not attributed to any single app, so the per-app `traced` column above sums to less. A rate over the subset you looked at is not a rate.
...
|---|---|---|---|
| `suites` | 335 | UNDER-counts | test files on disk attributed to no single app by path |
| `traced` | 15 | UNDER-counts | test files no source ties to a stated requirement |
...
**WORST CASE: 379.** RSS for context: 337.
...
```
  ...
  test files on disk               517   tests/**, api/*.test.js
  tests traced to a requirement    508   traceability_matrix.traced()
  declared fault probes             50   MUTATIONS blocks + *_fault_probe.py + *_mutation_control.js + tests/faults/*.js
