...
**Platform totals: 387 resources owned by an app, 183 test files attributed to one, 178 of those traced, 41 fault probes.**

**And the denominators that are NOT the same thing**, stated because conflating them is what went wrong: there are **519** test files on disk in total and **502** of them are traced — most are not attributed to any single app, so the per-app `traced` column above sums to less. A rate over the subset you looked at is not a rate.
...
|---|---|---|---|
| `suites` | 336 | UNDER-counts | test files on disk attributed to no single app by path |
| `traced` | 17 | UNDER-counts | test files no source ties to a stated requirement |
...
**WORST CASE: 382.** RSS for context: 338.
...
```
  ...
  test files on disk               519   tests/**, api/*.test.js
  tests traced to a requirement    508   traceability_matrix.traced()
  declared fault probes             52   MUTATIONS blocks + *_fault_probe.py + *_mutation_control.js + tests/faults/*.js
