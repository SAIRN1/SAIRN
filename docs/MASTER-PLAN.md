...
**Platform totals: 387 resources owned by an app, 181 test files attributed to one, 177 of those traced, 38 fault probes.**

**And the denominators that are NOT the same thing**, stated because conflating them is what went wrong: there are **516** test files on disk in total and **501** of them are traced — most are not attributed to any single app, so the per-app `traced` column above sums to less. A rate over the subset you looked at is not a rate.
...
```
  ...
  declared fault probes             49   MUTATIONS blocks + *_fault_probe.py + *_mutation_control.js + tests/faults/*.js
