...
**Platform totals: 386 resources owned by an app, 160 test files attributed to one, 73 of those traced, 25 fault probes.**

**And the denominators that are NOT the same thing**, stated because conflating them is what went wrong: there are **435** test files on disk in total and **225** of them are traced — most are not attributed to any single app, so the per-app `traced` column above sums to less. A rate over the subset you looked at is not a rate.
...
```
  ...
  test files on disk               435   tests/**, api/*.test.js
