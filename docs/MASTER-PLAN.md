...
**Platform totals: 387 resources owned by an app, 229 test files attributed to one, 207 of those traced, 55 fault probes.**

**And the denominators that are NOT the same thing**, stated because conflating them is what went wrong: there are **618** test files on disk in total and **558** of them are traced — most are not attributed to any single app, so the per-app `traced` column above sums to less. A rate over the subset you looked at is not a rate.
...
|---|---|---|---|
| `suites` | 389 | UNDER-counts | test files on disk attributed to no single app by path |
...
**WORST CASE: 478.** RSS for context: 395.
