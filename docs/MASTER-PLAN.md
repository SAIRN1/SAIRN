...
**Platform totals: 387 resources owned by an app, 184 test files attributed to one, 179 of those traced, 43 fault probes.**

**And the denominators that are NOT the same thing**, stated because conflating them is what went wrong: there are **522** test files on disk in total and **503** of them are traced — most are not attributed to any single app, so the per-app `traced` column above sums to less. A rate over the subset you looked at is not a rate.
...
|---|---|---|---|
| `suites` | 338 | UNDER-counts | test files on disk attributed to no single app by path |
| `traced` | 19 | UNDER-counts | test files no source ties to a stated requirement |
...
**WORST CASE: 386.** RSS for context: 340.
...
```
  ...
  test files on disk               522   tests/**, api/*.test.js
  tests traced to a requirement    509   traceability_matrix.traced()
  declared fault probes             54   MUTATIONS blocks + *_fault_probe.py + *_mutation_control.js + tests/faults/*.js
