...
**Platform totals: 386 resources owned by an app, 153 test files attributed to one, 67 of those traced, 16 fault probes.**

**And the denominators that are NOT the same thing**, stated because conflating them is what went wrong: there are **421** test files on disk in total and **216** of them are traced — most are not attributed to any single app, so the per-app `traced` column above sums to less. A rate over the subset you looked at is not a rate.
...
|---|---|---|---|
| `suites` | 268 | UNDER-counts | test files on disk attributed to no single app by path |
...
**WORST CASE: 501.** RSS for context: 339.
...
```
  ...
  test files on disk               421   tests/**, api/*.test.js
  tests traced to a requirement    222   traceability_matrix.traced()
  declared fault probes             17   MUTATIONS blocks + *_fault_probe.py
