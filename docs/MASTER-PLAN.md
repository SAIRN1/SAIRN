...
**Platform totals: 386 resources owned by an app, 157 test files attributed to one, 69 of those traced, 17 fault probes.**

**And the denominators that are NOT the same thing**, stated because conflating them is what went wrong: there are **427** test files on disk in total and **219** of them are traced — most are not attributed to any single app, so the per-app `traced` column above sums to less. A rate over the subset you looked at is not a rate.
...
**WORST CASE: 506.** RSS for context: 342.
...
```
  ...
  test files on disk               427   tests/**, api/*.test.js
