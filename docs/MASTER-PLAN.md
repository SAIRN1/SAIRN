...
**Platform totals: 387 resources owned by an app, 169 test files attributed to one, 114 of those traced, 29 fault probes.**

**And the denominators that are NOT the same thing**, stated because conflating them is what went wrong: there are **488** test files on disk in total and **385** of them are traced — most are not attributed to any single app, so the per-app `traced` column above sums to less. A rate over the subset you looked at is not a rate.
...
|---|---|---|---|
| `suites` | 319 | UNDER-counts | test files on disk attributed to no single app by path |
...
**WORST CASE: 451.** RSS for context: 336.
...
```
  ...
  tests traced to a requirement    391   traceability_matrix.traced()
