...
|---|---|---|---|
| `suites` | 330 | UNDER-counts | test files on disk attributed to no single app by path |
| `traced` | 9 | UNDER-counts | test files no source ties to a stated requirement |
...
**WORST CASE: 368.** RSS for context: 331.
...
```
  ...
  test files on disk               507   tests/**, api/*.test.js
  tests traced to a requirement    504   traceability_matrix.traced()
  declared fault probes             43   MUTATIONS blocks + *_fault_probe.py + *_mutation_control.js + tests/faults/*.js
