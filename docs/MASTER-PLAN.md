...
**Platform totals: 386 resources owned by an app, 154 test files attributed to one, 67 of those traced, 17 fault probes.**

**And the denominators that are NOT the same thing**, stated because conflating them is what went wrong: there are **422** test files on disk in total and **216** of them are traced — most are not attributed to any single app, so the per-app `traced` column above sums to less. A rate over the subset you looked at is not a rate.
...
**WORST CASE: 502.** RSS for context: 339.
