...
| `fault` | 21 | UNDER-counts | test files that write to a tracked app file and declare neither a MUTATIONS block nor a *_fault_probe.py name |
| `tiered` | 0 | no contribution | binary and complete -- criticality_tier_check either raised something for an app or did not |

**WORST CASE: 492.** RSS for context: 338.
...
```
  ...
  tests traced to a requirement    159   traceability_matrix.traced()
