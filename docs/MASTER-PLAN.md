...
**Platform totals: 382 resources owned by an app, 76 test files attributed to one, 30 of those traced, 8 fault probes.**

**And the denominators that are NOT the same thing**, stated because conflating them is what went wrong: there are **352** test files on disk in total and **157** of them are traced — most are not attributed to any single app, so the per-app `traced` column above sums to less. A rate over the subset you looked at is not a rate.
...
|---|---|---|---|
| `suites` | 276 | UNDER-counts | test files on disk attributed to no single app by path |
...
**WORST CASE: 493.** RSS for context: 339.
...
```
  ...
  tests traced to a requirement    162   traceability_matrix.traced()
