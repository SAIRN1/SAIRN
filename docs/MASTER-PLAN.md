...
**Platform totals: 387 resources owned by an app, 166 test files attributed to one, 78 of those traced, 28 fault probes.**

**And the denominators that are NOT the same thing**, stated because conflating them is what went wrong: there are **478** test files on disk in total and **281** of them are traced — most are not attributed to any single app, so the per-app `traced` column above sums to less. A rate over the subset you looked at is not a rate.
...
|---|---|---|---|
| `suites` | 312 | UNDER-counts | test files on disk attributed to no single app by path |
...
**WORST CASE: 538.** RSS for context: 370.
...
```
  ...
  tests traced to a requirement    287   traceability_matrix.traced()
