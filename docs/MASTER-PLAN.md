...
|---|---|---|---|
| `suites` | 293 | UNDER-counts | test files on disk attributed to no single app by path |
...
**WORST CASE: 534.** RSS for context: 363.
...
```
  ...
  test files on disk               456   tests/**, api/*.test.js
  tests traced to a requirement    249   traceability_matrix.traced()
