...
**Platform totals: 387 resources owned by an app, 163 test files attributed to one, 74 of those traced, 26 fault probes.**

**And the denominators that are NOT the same thing**, stated because conflating them is what went wrong: there are **465** test files on disk in total and **250** of them are traced — most are not attributed to any single app, so the per-app `traced` column above sums to less. A rate over the subset you looked at is not a rate.
...
|---|---|---|---|
| `suites` | 302 | UNDER-counts | test files on disk attributed to no single app by path |
| `traced` | 215 | UNDER-counts | test files no source ties to a stated requirement |
...
**WORST CASE: 546.** RSS for context: 372.
...
```
  ...
  test files on disk               465   tests/**, api/*.test.js
  tests traced to a requirement    256   traceability_matrix.traced()
  declared fault probes             27   MUTATIONS blocks + *_fault_probe.py + *_mutation_control.js + tests/faults/*.js
