...
**Platform totals: 387 resources owned by an app, 177 test files attributed to one, 174 of those traced, 33 fault probes.**

**And the denominators that are NOT the same thing**, stated because conflating them is what went wrong: there are **508** test files on disk in total and **498** of them are traced — most are not attributed to any single app, so the per-app `traced` column above sums to less. A rate over the subset you looked at is not a rate.
...
|---|---|---|---|
| `suites` | 331 | UNDER-counts | test files on disk attributed to no single app by path |
| `traced` | 10 | UNDER-counts | test files no source ties to a stated requirement |
...
**WORST CASE: 370.** RSS for context: 332.
...
```
  ...
  test files on disk               508   tests/**, api/*.test.js
  tests traced to a requirement    504   traceability_matrix.traced()
  declared fault probes             44   MUTATIONS blocks + *_fault_probe.py + *_mutation_control.js + tests/faults/*.js
