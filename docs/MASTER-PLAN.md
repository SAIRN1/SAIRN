...
|---|---|---|---|
| `suites` | 325 | UNDER-counts | test files on disk attributed to no single app by path |
...
**WORST CASE: 359.** RSS for context: 326.
...
```
  ...
  tests traced to a requirement    501   traceability_matrix.traced()
  declared fault probes             38   MUTATIONS blocks + *_fault_probe.py + *_mutation_control.js + tests/faults/*.js
