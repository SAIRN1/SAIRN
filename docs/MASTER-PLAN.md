...
|---|---|---|---|
| `suites` | 305 | UNDER-counts | test files on disk attributed to no single app by path |
| `traced` | 218 | UNDER-counts | test files no source ties to a stated requirement |
...
**WORST CASE: 552.** RSS for context: 376.
...
```
  ...
  test files on disk               471   tests/**, api/*.test.js
  tests traced to a requirement    259   traceability_matrix.traced()
