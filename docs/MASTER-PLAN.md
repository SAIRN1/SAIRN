...
**Platform totals: 386 resources owned by an app, 161 test files attributed to one, 74 of those traced, 26 fault probes.**

**And the denominators that are NOT the same thing**, stated because conflating them is what went wrong: there are **437** test files on disk in total and **228** of them are traced — most are not attributed to any single app, so the per-app `traced` column above sums to less. A rate over the subset you looked at is not a rate.
...
| `traced` | 209 | UNDER-counts | test files no source ties to a stated requirement |
...
**WORST CASE: 512.** RSS for context: 347.
...
```
  ...
  tests traced to a requirement    234   traceability_matrix.traced()
