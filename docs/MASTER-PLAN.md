...
|---|---|---|---|
| `suites` | 311 | UNDER-counts | test files on disk attributed to no single app by path |
...
**WORST CASE: 537.** RSS for context: 369.
...
```
  ...
  test files on disk               477   tests/**, api/*.test.js
  tests traced to a requirement    286   traceability_matrix.traced()
