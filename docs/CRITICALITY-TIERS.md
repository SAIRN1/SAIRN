...
| `sairnbuild` | 32 | **6** | 26 | 0 | **RE-TIERED** — `bld_bids`, `bld_change_orders`, `bld_costs`, `bld_incidents`, `bld_price_points`, `bld_sub_bids` |
...
| `stonedesk` | 36 | **11** | 24 | 1 | **RE-TIERED** — invoicing, financial jobs, pricing rules, negotiated prices, quote history, AI quotes, order history, public quote requests, employee records, certifications, **the private executive channel (2026-09-21, B&rarr;A on a documented failure)** |
...
- **The B tier is 296 rows and it is the honest weak point of this file.** Each says the same thing — auth-gated, not money, not regulated — because that is what the rule says, not because 296 files were read. **`bld_change_orders` is the second row in two days to be found carrying money under that sentence**, and unlike `sd_exec_msgs` it was found by reading the app rather than by an incident — which is the cheaper of the two ways and the only one that scales. A resource misfiled as B is the failure mode that matters, and the only defence here is that the A criteria are broad on purpose: money *and* anything upstream of money, regulated data *and* licensure, plus any resource with a documented incident.
  ...
  - **This count is prose and the tool does not check it.** It read 299 against an actual 298 before the re-tier above — already stale by one when I got here, from some earlier row move that corrected the table and not the sentence. Re-derive it from the table when a tier changes rather than adjusting it by the size of your own edit: `python -c "import re;print(__import__('collections').Counter(re.findall(r'^\| \`[^\`]+\` \| \*\*([ABC])\*\* \|', open('docs/CRITICALITY-TIERS.md',encoding='utf-8').read(), re.M)))"`.
