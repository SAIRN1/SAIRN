...
| 65b | A recorded **row-count baseline per table**, taken on a cadence from production, so a restore can be compared against something rather than only against itself. **WRITER BUILT 2026-09-25: `tools/row_count_baseline.js`, 25 arms in `tests/run_row_count_baseline_probe.js`, sabotage-verified.** A table that 404s or errors is OMITTED, never written as 0 — a zero here would make a restore that lost every row in that table compare EQUAL | **BUILT** | **the CAPTURE is blocked on Michael** — it needs production credentials (`SAIRN_BASELINE_URL`/`SAIRN_BASELINE_KEY`), deliberately NOT the checker's variable names |
