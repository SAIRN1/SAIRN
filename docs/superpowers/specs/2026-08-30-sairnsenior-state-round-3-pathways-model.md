...
| Arizona (`apps.azsos.gov` 403), Tennessee (`publications.tnsosfiles.com` 403), Maryland (`dsd.maryland.gov` 404), Seattle DWO (404) | **NOT RETRIEVED** | These four resisted even the full header set — so it is not one universal trick; each needs its own route. |
| Ohio 3701-60-04, Colorado 6 CCR 1011-1 | **FETCHED, NOT YET READ** | Both returned 200 and are on disk. Cheap next step. |
