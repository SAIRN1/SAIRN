...
| B2 | **Retainage holdback / release with a full audit trail** | Retainage is money owed that is deliberately withheld; release is a contractual event that gets disputed | **~~HOLDBACK BUILT, RELEASE ABSENT~~ — BUILT 2026-09-15 (Hank), `e2624577`.** As written this row was correct: "Retainage Held" KPI, per-draw column, and a default retainage setting that pre-fills new draws — but **zero occurrences of any retainage release path, and no audit trail on it.** Money can go in and never come out. **Now it can come out:** `release_retainage` on `bld_draws` in `api/sd-data.js`, delegating every rule to `api/_lib/wip-accounting.js` rather than growing a second model, with a who/when/how-much trail and a refusal to go backwards; the KPI reads OUTSTANDING and the gross total sits under it. **STATUS CAVEAT, and it is the reason this cell is not a plain BUILT:** every refusal is live-verified against the real deployment, but **no release has ever landed in a real row** — `sql/sairnbuild_data_schema.sql` has never been run in Supabase, so the happy path is proven only against a stubbed one. See the SAIRNbuild row in `docs/SAIRN-OPEN-WORK-INDEX.md` |
