...
| A5 | **Imaging / sensor / CBCT / intraoral-scanner integration** | The single most expected integration surface in dentistry. DEXIS, Carestream, Planmeca, 3Shape TRIOS. Notably: **the only genuinely open documented API found anywhere in dental was NexHealth's**; everything else is a curated partner marketplace. **CORRECTED 2026-09-25 (Cody), external research PR #17: this is OUT OF DATE. DEXIS publishes public API documentation**, so NexHealth is no longer the only open documented API in this category and the "everything else is a curated partner marketplace" half does not hold as stated. The ORIGINAL SENTENCE IS LEFT ABOVE rather than overwritten, because it was true of what the 2026-08-26 survey could reach and the correction is about the market moving, not about that survey being careless. **NOT INDEPENDENTLY FETCHED FROM HERE** &mdash; this correction rests on PR #17's research, which is one grade below reading the vendor's own page, so it is recorded as "the claim is no longer safe to repeat" rather than as a verified new fact. **What does NOT change: SAIRNdental is still zero for imaging/radiograph**, and an available API makes this gap buildable-in-principle rather than built.** | **Zero for imaging / radiograph.** SAIRNdental has photo capture, which is not the same category. |
