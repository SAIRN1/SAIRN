...
## Correction (2026-08-12, final whole-branch review)

This spec's Architecture section claimed `dcCanvasCoords()` is the single, universal screen-to-canvas-pixel conversion point every existing hit-test/drag function already goes through — that claim was **false** for three functions: `dcPresetEdgeDragMove()`, `dcPresetEdgeDragEnd()`, and `dcCanvasPlaceOrDrag()` (the last of which this spec explicitly named as already routing through it). Each inlined its own uncorrected copy of the raw screen-to-buffer conversion instead, bypassing the zoom inverse entirely. Because none of them were touched by the plan built from this spec, no per-task review's diff ever contained them, and all three task reviews correctly approved code that was, in aggregate, incomplete. Caught only by the final whole-branch review, which read the untouched call sites those functions' callers pass through. All three were fixed directly (routed through `dcCanvasCoords()`), along with two dependent overlay-positioning functions (`dcShowDragOverlay()`, `dcOpenEdgeTypeMenu()`) that needed a matching forward-transform fix. Recorded here so a future feature touching this canvas doesn't inherit the same false premise from this document.
