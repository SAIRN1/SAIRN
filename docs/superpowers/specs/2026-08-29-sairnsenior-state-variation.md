...
| MA home care worker registry / 105 CMR | **STILL UNRESOLVED, and the reason changed** | Prior pass recorded a mass.gov landing shell (inconclusive). Re-tested today per the "never record a host as blocked without re-testing" rule: the regulation URL returned **HTTP 403**, not a shell. So today's obstacle is a bot wall on that path, not an empty page. Different remedy — find a non-mass.gov primary route (Secretary of the Commonwealth CMR publication) rather than re-fetch. |
| TX HCSSA licensure and personal-assistant training (26 TAC ch. 558) | **NOT RETRIEVABLE by current method** | `texreg.sos.state.tx.us` now serves only a redirect notice; the replacement is an **Appian portal** that renders via JavaScript and returns no regulatory text to a fetch. Needs a browser-driven read or a different publisher. |
...
| State domestic-worker overtime rules | **NOT STARTED** | Carried forward unchanged from `2026-08-28…` §7 item 5. These survive a federal FLSA rescission, so they are the higher-value half of the wage question. |
