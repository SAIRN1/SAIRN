...
This plan's Global Constraint "No existing hit-test/drag function ... may be modified — the zoom inverse lives entirely inside `dcCanvasCoords()`, which they already all call" was **false** for `dcPresetEdgeDragMove()`, `dcPresetEdgeDragEnd()`, and `dcCanvasPlaceOrDrag()` (the last named explicitly in that same constraint as already compliant). None of Tasks 1-3 touched these functions, so no task review's diff ever contained them — the false premise survived three clean task reviews and was only caught by the final whole-branch review. All three were fixed directly after the final review (routed through `dcCanvasCoords()`), plus two dependent overlay-positioning functions. See the design spec's own matching Correction section for the full detail. Lesson for future plans on this file: verify a "single universal conversion point" claim by grep, not by reading the one function that defines it.
