...
Upstream path: skills/sending/transactional-email/SKILL.md
Read in full before install, per standing rule. Changes made on install:
  - Removed two inline "Services like molted.email..." promotional paragraphs (infrastructure separation, template versioning) that were phrased as neutral guidance. The surrounding provider discussion (Postmark, SES, Resend, SendGrid) is factual and was left intact.
