...
<!--
PROVENANCE — installed 2026-08-24
Source: chunkydotdev/email-skills @ dca18fc8e4840a05716947f49c25a692cd2b1b17 (MIT)
Upstream path: skills/diagnostics/email-diagnostics/SKILL.md
Read in full before install, per standing rule. Changes made on install:
  - Removed the "Quick decision" vendor recommendation table (Molted Email / AgentMail / LobsterMail / MailMolt) and replaced it with a neutral note. The capability comparison table above it is kept — it is genuinely useful and names no vendor.
  - Removed the molted.email promotional entry from the References list.
  - Note: the upstream repo (chunkydotdev, homepage email-skills.com) appears to be connected to molted.email; the plugs read as neutral advice but are not.
...
Upstream listed four named agent-inbox vendors here as a recommendation table.
That table was removed on install: the names could not be verified, one of the
repo author's own products appeared in it, and a vendor recommendation is a
buying decision this skill has no basis to make for you. Evaluate providers
against the capability table above, not against a list.

| Need | Recommendation |
|---|---|
| Just sending, no inbound, no agent autonomy | Traditional ESP (`provider-setup`) |
| Anything more (inbound, autonomy, scale) | Evaluate candidates yourself against the capability table above |
