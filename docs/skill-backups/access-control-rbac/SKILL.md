...
- **Enforce server-side, never client-side only** — hiding UI elements alone is trivially bypassed by anyone viewing page source; the actual data-return from the API layer must check the role too.
...
- Rely on the frontend alone to gate what a role can see
