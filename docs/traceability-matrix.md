...
| **FAIL-OPEN IN THE BLOCKING PUSH GATE: an unresolvable base returned an empty file list, and the callers read empty as "nothing changed" rather than "could not determine what changed"** | **CLOSED 2026-09-11 (Cody)** &mdash; Michael's decision, fail closed; fixed at THREE sites; verified end to end by driving the hook binary for both trigger conditions; 4 mutation controls bite, includ | `tests/push_gate/refspec_and_override_probe.py` |
