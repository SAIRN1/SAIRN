...
| Requirement | Status | Proved by |
|---|---|---|
| The generic `DNT_RESOURCES` write validates `payload.id` and nothing else &mdash; **SEVENTEEN resources, not fifteen** | Open &mdash; **12 of 17 CLOSED, FIVE REMAIN &mdash; and TWO of those five are DELIBERATELY UNWRITTEN, so the real remainder is THREE.** `dnt_recall_outreach`, `dnt_referrals`, `dnt_providers`, `dnt_pr | `api/sd-data-dental-ledger-validation.test.js`, `api/sd-data-dental-provider-scope.test.js` |
