...
## The 12 cross-cutting rules — apply to every task, no exceptions
...
12. **Documentation/training content must come from real code inspection, never memory.** Tried to generate StoneDesk's real panel list from my own recollection of a long night's changes — genuinely risky given the volume of real edits made. The right source is always direct code access (Hank), never a summary reconstructed from memory, for anything meant to train a real person or represent current app behavior.
