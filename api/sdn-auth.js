// api/sdn-auth.js
// ---------------------------------------------------------------------------
// SAIRNdesign per-employee login + lockout endpoint.
//
// Replaces sairndesign.html's old client-only gate (DEFAULT_PINS = one
// shared PIN per role, in the page source, zero server involvement — every
// designer shared one PIN, so nothing could ever be attributed to an
...
// hashed server-side, checked against sairndesign_employee_auth
// (sql/sairndesign_employee_auth_schema.sql).
//
// Modelled directly on api/leg-auth.js (SAIRNlegacy's equivalent, built
// 2026-08-19 for the same reason). Deliberately SMALLER than law-auth.js:
// no MFA, no SSO, no audit-log table — none of that was asked for. The
// 'roster' action (read-only, management-only) is the one addition beyond
// SAIRNlegacy's own action set — backs the Assign-To/reassign controls
// for the client/lead privacy gate (api/sd-data.js's sdn_clients branch),
// same shape as StoneDesk's api/sd-auth.js 'roster' action built the same
...
//     just the token's own unexpired signature -- same fix SAIRNlaw
//     needed live (2026-08-18), applied here from the start.
//
...
//
//   roster         {}                      (owner/office session -- management)
//     Read-only list of active employees (employee_id/display_name/role)
//     for the Assign-To/reassign dropdowns. Never returns PIN hashes/salts/
//     lockout state.
//
...
const TABLE = 'sairndesign_employee_auth';
const SDN_ROLES = ROLES_BY_APP[APP];
...
module.exports = async (req, res) => {
  ...
  if (!SUPABASE_URL || !SERVICE_KEY || !process.env.SD_AUTH_SECRET) {
    console.error('sdn-auth: SUPABASE_URL / SUPABASE_SERVICE_ROLE_KEY / SD_AUTH_SECRET not set');
  ...
  try {
    ...
    if (action === 'bootstrap') {
      ...
      if (Array.isArray(existingRows) && existingRows.length > 0) {
        res.status(409).json({ error: { code: 'ALREADY_PROVISIONED', message: 'This studio already has employee credentials set up — use action:setup instead' } });
        return;
      }
    ...
    if (action === 'setup') {
      ...
      if (!employee_id || employee_id.length > 128 || !/^\d{6,8}$/.test(pin) || SDN_ROLES.indexOf(role) === -1) {
        res.status(400).json({ error: { message: 'employee_id (max 128 chars), a 6-8 digit pin, and a valid role (' + SDN_ROLES.join('|') + ') are required' } });
        return;
      }
      if (body.display_name && String(body.display_name).length > 128) {
        res.status(400).json({ error: { message: 'display_name max 128 chars' } });
    ...
    // employee becomes selectable again. sairndesign.html's consumers were updated in the
    // same commit; `_sdnRoster` is the grep that finds them.
    if (action === 'roster') {
      const caller = verifySessionToken(tokenFromRequest(req), licHash, APP);
    ...
    if (err && err.notProvisioned) {
      res.status(503).json({ error: { code: 'NOT_PROVISIONED', message: 'SAIRNdesign employee accounts are not set up yet — run sql/sairndesign_employee_auth_schema.sql' } });
      return;
    }
    console.error('api/sdn-auth error:', err);
...
function upstream(res, detail) {
  console.error('sdn-auth upstream error:', detail);
  if (isMissingTable(detail)) {
    res.status(503).json({ error: { code: 'NOT_PROVISIONED', message: 'SAIRNdesign employee accounts are not set up yet — run sql/sairndesign_employee_auth_schema.sql' } });
    return;
  }
  if (isPermissionDenied(detail)) {
    res.status(503).json({ error: { code: 'NOT_GRANTED', message: 'The SAIRNdesign tables exist but the server role lacks privileges on them — re-run the GRANT block at the end of sql/sairndesign_employee_auth_schema.sql' } });
    return;
  }
