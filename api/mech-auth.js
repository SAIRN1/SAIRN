// api/mech-auth.js
// ---------------------------------------------------------------------------
// SAIRNmechanical per-employee login, lockout, and credential lifecycle.
//
// RECOVERY BUILD 2026-08-27 -- SAIRNmechanical shipped with a client-only PIN
...
//
// ROLE MODEL -- the app's OWN four, not invented. sairnmechanical.html already
// used owner / tech / sales / admin as the keys of its client-side PIN object;
// this build reuses those exact names so nothing in the UI has to be relearned
// and no unused permission surface is created.
//   owner -- principal. Provisions credentials.
//   admin -- office manager. Reads the roster, does NOT mint identities.
//   sales -- quoting and customer-facing work.
//   tech  -- field technician.
//
// PROVISIONING is owner-only, matching SAIRNroofing rather than StoneDesk:
// a trade shop has one principal and the blast radius of a mistaken
// deactivation is the whole company.
//
// ⚠ NO SERVER DATA LAYER YET. SAIRNmechanical has no MECH_RESOURCES branch in
// api/sd-data.js; its storage is localStorage plus one unauthenticated
// api/bridge.js push. So this endpoint gates the APP'S OWN LOGIN and there is
// not yet protected data behind it. Recorded plainly so nobody reads "auth
// shipped" as "the data is secured" -- when a data layer is added, every
// branch of it needs verifySessionToken(token, licHash, 'sairnmechanical')
...
// REQUIRES sql/sairnmechanical_employee_auth_schema.sql to have been run.
// ---------------------------------------------------------------------------
...
const TABLE = 'sairnmechanical_employee_auth';
const MECH_ROLES = ROLES_BY_APP[APP];
...
// Exported so gates elsewhere (api/sd-data.js, if a data layer is added) import these
...
module.exports = async (req, res) => {
  ...
  if (!SUPABASE_URL || !SERVICE_KEY || !process.env.SD_AUTH_SECRET) {
    console.error('mech-auth: SUPABASE_URL / SUPABASE_SERVICE_ROLE_KEY / SD_AUTH_SECRET not set');
  ...
  try {
    ...
    if (action === 'setup') {
      ...
      if (!employee_id || employee_id.length > 128 || !/^\d{6,8}$/.test(pin) || MECH_ROLES.indexOf(role) === -1) {
        res.status(400).json({ error: { message: 'employee_id (max 128 chars), a 6-8 digit pin, and a valid role (' + MECH_ROLES.join('|') + ') are required' } });
        return;
      }
      if (body.display_name && String(body.display_name).length > 128) {
        res.status(400).json({ error: { message: 'display_name max 128 chars' } });
    ...
    if (action === 'roster') {
      ...
      if (!caller || !MANAGEMENT_ROLES[caller.role]) {
        res.status(403).json({ error: { code: 'FORBIDDEN', message: 'Only an Owner or Admin can view the employee roster' } });
        return;
      }
    ...
    if (action === 'set_active') {
      ...
      // NO AUDIT LOG YET, stated rather than silently absent: SAIRNmechanical has
    ...
    if (err && err.notProvisioned) {
      res.status(503).json({ error: { code: 'NOT_PROVISIONED', message: 'SAIRNmechanical employee accounts are not set up yet — run sql/sairnmechanical_employee_auth_schema.sql' } });
      return;
    }
    console.error('api/mech-auth error:', err);
...
function upstream(res, detail) {
  console.error('mech-auth upstream error:', detail);
  if (isMissingTable(detail)) {
    res.status(503).json({ error: { code: 'NOT_PROVISIONED', message: 'SAIRNmechanical employee accounts are not set up yet — run sql/sairnmechanical_employee_auth_schema.sql' } });
    return;
  }
  if (isPermissionDenied(detail)) {
    res.status(503).json({ error: { code: 'NOT_GRANTED', message: 'The SAIRNmechanical tables exist but the server role lacks privileges on them — re-run the GRANT block at the end of sql/sairnmechanical_employee_auth_schema.sql' } });
    return;
  }
