// api/bld-auth.js
// ---------------------------------------------------------------------------
// SAIRNbuild per-employee login + lockout endpoint.
//
// Built from ZERO -- unlike api/sd-auth.js/leg-auth.js/sdn-auth.js (each of
// which replaced an existing client-only PIN gate for an app that already
// had that scaffold), SAIRNbuild had no server auth at all before this.
// Replaces sairnbuild.html's DEFAULT_PINS = one shared PIN per role, in the
// page source, zero server involvement -- every PM shared one PIN, so
// nothing could ever be attributed to an individual and any role was
// self-assertable by editing the DOM. This is the server side of the real
// model: each employee has their own PIN, hashed server-side, checked
// against sairnbuild_employee_auth (sql/sairnbuild_employee_auth_schema.sql).
//
// Modelled directly on api/sdn-auth.js (SAIRNdesign's equivalent, built
...
// the Bids & Proposals privacy gate (api/sd-data.js's bld_bids branch),
// same shape as StoneDesk's api/sd-auth.js and SAIRNdesign's api/sdn-auth.js
// 'roster' actions.
//
...
const TABLE = 'sairnbuild_employee_auth';
...
module.exports = async (req, res) => {
  ...
  if (!SUPABASE_URL || !SERVICE_KEY || !process.env.SD_AUTH_SECRET) {
    console.error('bld-auth: SUPABASE_URL / SUPABASE_SERVICE_ROLE_KEY / SD_AUTH_SECRET not set');
  ...
  try {
    ...
    if (action === 'bootstrap') {
      ...
      if (Array.isArray(existingRows) && existingRows.length > 0) {
        res.status(409).json({ error: { code: 'ALREADY_PROVISIONED', message: 'This account already has employee credentials set up — use action:setup instead' } });
        return;
      }
    ...
    if (action === 'setup') {
      ...
      if (!employee_id || employee_id.length > 128 || !/^\d{6,8}$/.test(pin) || BLD_ROLES.indexOf(role) === -1) {
        res.status(400).json({ error: { message: 'employee_id (max 128 chars), a 6-8 digit pin, and a valid role (' + BLD_ROLES.join('|') + ') are required' } });
        return;
      }
      if (body.display_name && String(body.display_name).length > 128) {
        res.status(400).json({ error: { message: 'display_name max 128 chars' } });
    ...
    if (err && err.notProvisioned) {
      res.status(503).json({ error: { code: 'NOT_PROVISIONED', message: 'SAIRNbuild employee accounts are not set up yet — run sql/sairnbuild_employee_auth_schema.sql' } });
      return;
    }
    console.error('api/bld-auth error:', err);
...
function upstream(res, detail) {
  console.error('bld-auth upstream error:', detail);
  if (isMissingTable(detail)) {
    res.status(503).json({ error: { code: 'NOT_PROVISIONED', message: 'SAIRNbuild employee accounts are not set up yet — run sql/sairnbuild_employee_auth_schema.sql' } });
    return;
  }
  if (isPermissionDenied(detail)) {
    res.status(503).json({ error: { code: 'NOT_GRANTED', message: 'The SAIRNbuild tables exist but the server role lacks privileges on them — re-run the GRANT block at the end of sql/sairnbuild_employee_auth_schema.sql' } });
    return;
  }
