// api/sen-auth.js
// ---------------------------------------------------------------------------
// SAIRNsenior per-employee login + lockout endpoint.
//
// Built from ZERO -- SAIRNsenior is a ground-up app, never had a shared-PIN
// scaffold to replace, same starting point as SAIRNlegacy/SAIRNlaw/
// SAIRNbuild's real-auth builds. Real per-employee identity is the
// prerequisite for the HIPAA minimum-necessary client-visibility gate this
// app is built around (api/sd-data.js's sen_clients branch): a caregiver
// must only ever see their own assigned clients' health information.
//
// Modelled directly on api/bld-auth.js (SAIRNbuild's equivalent, built
...
// the client privacy gate, same shape as every other app's 'roster' action
// this session.
//
...
//
//   roster         {}                      (owner/billing session -- management)
...
module.exports = async (req, res) => {
  ...
  if (!SUPABASE_URL || !SERVICE_KEY || !process.env.SD_AUTH_SECRET) {
    console.error('sen-auth: SUPABASE_URL / SUPABASE_SERVICE_ROLE_KEY / SD_AUTH_SECRET not set');
  ...
  try {
    ...
    if (action === 'bootstrap') {
      ...
      if (Array.isArray(existingRows) && existingRows.length > 0) {
        res.status(409).json({ error: { code: 'ALREADY_PROVISIONED', message: 'This agency already has employee credentials set up — use action:setup instead' } });
        return;
      }
    ...
    if (action === 'setup') {
      ...
      if (!employee_id || employee_id.length > 128 || !/^\d{6,8}$/.test(pin) || SEN_ROLES.indexOf(role) === -1) {
        res.status(400).json({ error: { message: 'employee_id (max 128 chars), a 6-8 digit pin, and a valid role (' + SEN_ROLES.join('|') + ') are required' } });
        return;
      }
      if (body.display_name && String(body.display_name).length > 128) {
        res.status(400).json({ error: { message: 'display_name max 128 chars' } });
    ...
    if (err && err.notProvisioned) {
      res.status(503).json({ error: { code: 'NOT_PROVISIONED', message: 'SAIRNsenior employee accounts are not set up yet — run sql/sairnsenior_employee_auth_schema.sql' } });
      return;
    }
    console.error('api/sen-auth error:', err);
...
function upstream(res, detail) {
  console.error('sen-auth upstream error:', detail);
  if (isMissingTable(detail)) {
    res.status(503).json({ error: { code: 'NOT_PROVISIONED', message: 'SAIRNsenior employee accounts are not set up yet — run sql/sairnsenior_employee_auth_schema.sql' } });
    return;
  }
  if (isPermissionDenied(detail)) {
    res.status(503).json({ error: { code: 'NOT_GRANTED', message: 'The SAIRNsenior tables exist but the server role lacks privileges on them — re-run the GRANT block at the end of sql/sairnsenior_employee_auth_schema.sql' } });
    return;
  }
