// api/scp-auth.js
// ---------------------------------------------------------------------------
// SAIRNscape per-employee login/credential-provisioning endpoint.
//
// Mirror of api/grd-auth.js (SAIRNgrounds' equivalent) — same design,
// separate table (sql/scp_employee_auth_schema.sql) and separate role
// vocabulary (owner/crew_lead/office, api/_lib/auth.js's
// ROLES_BY_APP.sairnscape), per the platform's standing decision to keep
// every app's identity system distinct rather than merge them. Replaces
// sairnscape.html's Phase 1 simple role-picker (client-only, no server
// verification) with the same real, signed-token system every other app
// uses.
//
// Three actions, all POST, license key via Authorization: Bearer:
//
//   action: 'bootstrap'  { employee_id, display_name, pin }
//     Only works when this license has ZERO scp_employee_auth rows yet.
...
//     success. Same per-credential lockout as every other app's auth
//     endpoint (5 failed attempts -> 15min).
//
//   action: 'setup'      { employee_id, display_name, pin, role }
//     Provisions or updates ANOTHER employee's credential. Requires a valid
//     session token (X-SD-Auth header) belonging to an owner or crew_lead —
//     the two roles with operational authority, mirroring every other
//     app's two-role setup-permission pattern.
//
...
// `owner` and `crew_lead` BOTH provision, so counting the last-admin guard
// over PROVISIONING_ROLES let a crew_lead deactivate the last owner: two
...
// caller.role !== 'owner'`, so the surviving crew_lead cannot mint a
...
module.exports = async (req, res) => {
  ...
  if (!SUPABASE_URL || !SERVICE_KEY || !process.env.SD_AUTH_SECRET) {
    console.error('scp-auth: SUPABASE_URL / SUPABASE_SERVICE_ROLE_KEY / SD_AUTH_SECRET not set');
  ...
  try {
    if (action === 'bootstrap') {
      ...
      if (existing.status === 404 || existing.status === 400) {
        res.status(503).json({ error: { code: 'NOT_PROVISIONED', message: 'SAIRNscape employee credentials table is not set up yet — run sql/scp_employee_auth_schema.sql in Supabase first.' } });
        return;
      }
    ...
    if (action === 'setup') {
      ...
      if (!caller || (caller.role !== 'owner' && caller.role !== 'crew_lead')) {
        res.status(403).json({ error: { code: 'FORBIDDEN', message: 'Only Owner or Crew Lead can provision employee credentials' } });
        return;
      }
      const employee_id = String((body.employee_id || '')).trim();
      const pin = String((body.pin || '')).trim();
      ...
      const demote = await lifecycle.soleRoleDemotionRefusal({
        ...
        soleMessage: 'This is the only active Owner on this license. Changing their role '
          + 'would leave the company with none and lock everyone out with no '
    ...
    console.error('api/scp-auth error:', err);
    res.status(502).json({ error: { message: 'Upstream connection error — try again' } });
  }
};

function upstream(res, detail) {
  console.error('scp-auth upstream error:', detail);
