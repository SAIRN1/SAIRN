...
module.exports = async (req, res) => {
  ...
  // a valid envelope with a bad action, and read the verb list straight out of
  // the refusal -- on the endpoint that stores clearinghouse API keys. Same
  // shape as api/sd-data.js and api/sd-sub-data.js. The 405 and bearer-presence
  // checks stay above, because neither can say anything about what exists. The
  // cost is real and is not hidden: a junk key now costs one license_keys
  // lookup it did not before, and a malformed request from a bad licence
  // reports the licence, not the malformation.
  let body = req.body;
